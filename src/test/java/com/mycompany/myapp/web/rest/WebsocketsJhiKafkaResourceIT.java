...
class WebsocketsJhiKafkaResourceIT {
    ...
    void setup() {
        ...
        WebsocketsJhiKafkaResource kafkaResource = new WebsocketsJhiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/websockets-jhi-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/websockets-jhi-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
